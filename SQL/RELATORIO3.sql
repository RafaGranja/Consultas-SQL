SELECT
		...
		PRIORIDADE,
		REPROCESSAMENTO,
		CONVERT(DECIMAL(10, 0),PERCENTUAL_CONCLUIDO*100) PERCENTUAL_CONCLUIDO,
		HR_PREVISTAS,
		HR_APONTADAS,
		CASE WHEN HR_APONTADAS = 0 THEN CONVERT(DECIMAL(10, 2), HR_PREVISTAS)
			ELSE 
				CASE WHEN PERCENTUAL_CONCLUIDO = 0 AND HR_APONTADAS > 0 THEN 
					CASE WHEN HR_PREVISTAS > HR_APONTADAS THEN  CONVERT(DECIMAL(10, 2), HR_PREVISTAS )
					ELSE CONVERT(DECIMAL(10, 2), HR_APONTADAS) 
					END
				ELSE CONVERT(DECIMAL(10, 2),HR_APONTADAS/(CASE WHEN PERCENTUAL_CONCLUIDO > 1.00000 THEN  1.00000 ELSE PERCENTUAL_CONCLUIDO END)) 
				END
...
SELECT O.CODCCUSTO,O.DSCORDEM,
               ...
               ISNULL((SELECT convert(decimal(10,2),Sum(Round(Cast(Datediff(SECOND, K.DTHRINICIAL, K.DTHRFINAL) AS NUMERIC(15, 4)) / 3600, 4)))
        ...
        FROM   KATVORDEM A (NOLOCK) 
               ...
               INNER JOIN GCCUSTO C (NOLOCK) 
                  ...
                  INNER JOIN KATIVIDADE D (NOLOCK) 
                       ON D.CODCOLIGADA = A.CODCOLIGADA
                          AND D.CODFILIAL = A.CODFILIAL
                          AND D.CODATIVIDADE = A.CODATIVIDADE
               INNER JOIN KATVORDEMCOMPL KL (NOLOCK) 
                       ON KL.CODCOLIGADA=A.CODCOLIGADA
                          AND KL.CODFILIAL=A.CODFILIAL
                          AND KL.CODORDEM=O.CODORDEM
                          AND KL.CODESTRUTURA=A.CODESTRUTURA
                          AND KL.IDATVORDEM=A.IDATVORDEM
        ...
        WHERE  O.CODCCUSTO = :CCUSTO
               ...
               ORDER BY TAG,CODORDEM,PRIORIDADE,IDATVORDEM