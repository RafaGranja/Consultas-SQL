...
FROM   (SELECT CODCCUSTO,
               ...
               CASE
                 WHEN ( Sum(PERCENTUAL_CONCLUIDO) / Count(IDATVORDEM) ) = 0 THEN Sum(HR_PREVISTAS)
                 ELSE
                   CASE
                     WHEN ( Sum(PERCENTUAL_CONCLUIDO) / Count(IDATVORDEM) ) > 1.0000 THEN Sum(HR_APONTADAS)
                     ELSE Sum(HR_APONTADAS) / ( Sum(PERCENTUAL_CONCLUIDO) / Count(IDATVORDEM) )
                   END
               END               HORAS_PROJETADAS
        FROM   (SELECT O.CODCCUSTO,
                       C.CODCCUSTO                                           CCUSTOPOSTO,
                       C.NOME                                                NOMEPOSTO,
                       M.DESCRICAO,
                       A.CODCOLIGADA,
