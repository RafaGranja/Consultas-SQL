...
SELECT * FROM (
SELECT DISTINCT INDICE,KI.CODORDEM,EX1.CODIGOPRD,EX1.DESCRICAO,EX1.OS,EX1.CODTRFPAI,EX1.EXECUCAO,ISNULL(KI.QTDEEFETIVADA,0) QTDEEFETIVADA,KI.QTDEPLANEJADA,K.STATUS,KS.DSCSTATUS
	FROM FLUIG.DBO.Z_CRM_EX001005 (NOLOCK) EX1
		INNER JOIN KITEMORDEM KI (NOLOCK) ON
		KI.CODCOLIGADA=EX1.CODCOLIGADA
		AND KI.CODFILIAL=EX1.CODFILIAL
		...
		KS.CODCOLIGADA=KI.CODCOLIGADA
		AND KS.CODSTATUS=K.STATUS
	WHERE EX1.ITEMEXCLUSIVO<>2 AND EX1.NIVEL='' AND ( QTDEEFETIVADA<QTDEPLANEJADA OR KI.STATUS NOT IN (5,6))  AND COALESCE(K.REPROCESSAMENTO,0)=0 
	) R
	ORDER BY LEN(INDICE) DESC,CAST('/'+REPLACE(INDICE,'.','/')+'/' AS hierarchyid),CODORDEM
