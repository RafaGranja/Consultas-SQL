...
create queue [SingleExecQueue] 
go

create service [AsyncExecService] on queue [AsyncExecQueue] ([DEFAULT]);
go

create service [SingleExecService] on queue [SingleExecQueue] ([DEFAULT]);
go
...
create procedure [dbo].[usp_procedureInvokeHelper] (@x xml,@spid INT OUTPUT)
as
begin
...
UPDATE [AsyncExecResults] set spid=@@SPID where [token] = @token;exec ' + quotename(@x.value(N'(//procedure/name)[1]', N'sysname')) ;

    if (@namedParams != N'')
        select @stmt = @stmt + N' ' + @namedParams;

    exec sp_executesql @stmt, N'@x xml, @token uniqueidentifier', @x=@x,@token=@token ;
end
go
    ...
    begin try;
        ...
        begin
            ...
            begin
				...
				declare @RetryNo Int = 1,@RetryMaxNo Int = 3;
				while @RetryNo < @RetryMaxNo
				begin
					begin try
						exec [usp_procedureInvokeHelper] @xmlBody,@token;
					end try
					begin catch
						...
						if @execErrorNumber in (1204, 1205, 1222)
							begin
								while @@TRANCOUNT>0
								begin
									rollback;
						...
						begin
							--rollback;
							raiserror(N'Unrecoverable error in procedure: %i: %s Line : %i', 16, 10,
								@execErrorNumber, @execErrorMessage,@execErrorLine);
						end
						--else if (@xactState = 1)
						--begin
            ...
            begin
                ...
                select @token = [conversation_id],@finishTime = getdate()
                    from sys.conversation_endpoints with(nolock)
                    where [conversation_handle] = @h;
...
create procedure [dbo].[usp_SingleExecActivated]
as
begin
    set nocount on;
    declare @h uniqueidentifier
...
alter queue [AsyncExecQueue]
    ...
    , status = on);
go
alter queue [SingleExecQueue]
    with activation (
    procedure_name = [usp_SingleExecActivated]
    , max_queue_readers = 1
    , EXECUTE AS N'dbo'
    , status = on);
go
...
create procedure [usp_AsyncExecInvoke]
	...
	, @p6 sql_variant = NULL, @n6 sysname = NULL
	, @p7 sql_variant = NULL, @n7 sysname = NULL
	, @p8 sql_variant = NULL, @n8 sysname = NULL
	, @p9 sql_variant = NULL, @n9 sysname = NULL
	, @p10 sql_variant = NULL, @n10 sysname = NULL
    ...
    begin try
        ...
        select @xmlBody = (
            ...
            , (select * from (
				...
				union all select [dbo].[fn_DescribeSqlVariant] (@p6, @n6) AS [*]
                    WHERE @p6 IS NOT NULL
				union all select [dbo].[fn_DescribeSqlVariant] (@p7, @n7) AS [*]
                    WHERE @p7 IS NOT NULL
				union all select [dbo].[fn_DescribeSqlVariant] (@p8, @n8) AS [*]
                    WHERE @p8 IS NOT NULL
				union all select [dbo].[fn_DescribeSqlVariant] (@p9, @n9) AS [*]
                    WHERE @p9 IS NOT NULL
				union all select [dbo].[fn_DescribeSqlVariant] (@p10, @n10) AS [*]
        ...
        insert into [AsyncExecResults]
            ([token],[service_name],[submit_time],[xml_body])
            values
            (@token,@serviceName, getdate(),@xmlBody);
    if @trancount = 0
        commit;
    end try
    begin catch
        ...
        raiserror(N'Error: %i, %s  Linha %i', 16, 1, @error, @message,@lineError);
    end catch
end
;
