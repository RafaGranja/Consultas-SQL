           ...
           FROM (SELECT *,CASE
                        ...
                        WHEN COALESCE((SELECT IDPRDORIGEM
                                ...
                                KCOMPSUBSTITUTO
                                        ...
                                        IDPRODUTO
                                                AND
        CODESTRUTURA = R2.CODESTRUTURA AND CODATIVIDADE = R2.CODATIVIDADE), 0) = 0 THEN
                                (
                                SELECT
                                TOP 1 IDPRODUTO
                                FROM
...
AND C.OS=Z.OS
WHERE Z.OS=@OS 
--AND Z.EXECUCAO IN (SELECT DISTINCT PARAM FROM EXECUCAO) AND Z.CODTRFPAI=@CODTRFPAI 
AND Z.ITEMEXCLUSIVO <> 2 ) R
